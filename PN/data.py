...
class CIFAR_100():
    def __init__(self, batch_size, classes_per_set=10, samples_per_class=1, queries_per_class = 15, gray = False):
        ...
        self.datasets = {"train": self.x_train, "val": self.x_val, "test": self.x_test} #original data cached
        self.datasets_cache = {"train": self.load_data_cache(self.datasets["train"], self.samples_per_class, self.queries_per_class, self.classes_per_set),  #current epoch data cached
           "val": self.load_data_cache(self.datasets["val"], self.samples_per_class, self.queries_per_class, self.classes_per_set),
           "test": self.load_data_cache(self.datasets["test"], self.samples_per_class, self.queries_per_class, self.classes_per_set)}
    ...
    def load_data_cache(self, data_pack, samples_per_class, queries_per_class, classes_per_set):
        """
        ...
        n_queries = queries_per_class * classes_per_set
        data_cache = []
        for sample in range(1000):
            support_set_x = np.zeros((self.batch_size, n_samples, 32, 32, 3))
            support_set_y = np.zeros((self.batch_size, n_samples))
            target_x = np.zeros((self.batch_size, n_queries, 32, 32, 3), dtype=np.int)
            target_y = np.zeros((self.batch_size,n_queries), dtype=np.int)

            for i in range(self.batch_size):
                ind = 0
                iind = 0
                pinds = np.random.permutation(n_samples)
                qinds = np.random.permutation(n_queries)

                classes = np.random.choice(data_pack.shape[0], self.classes_per_set, False)
                for j, cur_class in enumerate(classes):  # each class
                    temp = np.arange(data_pack.shape[1])
                    np.random.shuffle(temp)
                    example_inds = temp[:samples_per_class]
                    example_pinds = temp[samples_per_class: samples_per_class+queries_per_class]
                    for eind in example_inds:
                        support_set_x[i, pinds[ind], :, :, :] = data_pack[cur_class][eind]
                        support_set_y[i, pinds[ind]] = j
                        ind += 1
                    for qind in example_pinds:
                        target_x[i, qinds[iind], :, :, :] = data_pack[cur_class][qind]
                        target_y[i, qinds[iind]] = j
                        iind += 1
    ...
    def get_batch(self, dataset_name):
        ...
        if self.indexes[dataset_name] >= len(self.datasets_cache[dataset_name]):
            ...
            self.datasets_cache[dataset_name] = self.load_data_cache(self.datasets[dataset_name], self.samples_per_class, self.queries_per_class, self.classes_per_set)
